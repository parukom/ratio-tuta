        ...
        try {
            const data = await toast.promise(
                ...
                })(),
                {
                    ...
                    success: (d: unknown) => (typeof d === 'object' && d && 'message' in d ? (d as { message?: string }).message || 'User updated' : 'User updated'),
                    error: (err: unknown) => (err instanceof Error ? err.message : 'Failed to update user'),
                },
            )
                                ...
                                <div className="flex-1">
                                        ...
                                        <div className="flex items-start justify-between space-x-3">
                                            <div className="space-y-1">
                                                ...
                                                <p className="text-sm text-gray-500 dark:text-gray-400">
                                                    Update basic information. Email changes will require re-verification.
                                                </p>
                                            </div>
                                    ...
                                    <div className="space-y-6 py-6 sm:space-y-0 sm:divide-y sm:divide-gray-200 sm:py-0 dark:sm:divide-white/10">
                                        <div className="space-y-2 px-4 sm:px-6 sm:py-5">
                                            <Input id="name" name="name" type="text" value={name} placeholder="Name" onChange={(e) => setName(e.target.value)} />
                                        </div>
                                        ...
                                        <div className="space-y-2 px-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:space-y-0 sm:px-6 sm:py-5">
                                            <div>
                                                <label className="block text-sm/6 font-medium text-gray-900 sm:mt-1.5 dark:text-white">Role</label>
                                            </div>
                                            <div className="sm:col-span-2">
                                                <Dropdown
                                                    buttonLabel={role === 'ADMIN' ? 'Admin' : 'Member'}
                                                    items={[
