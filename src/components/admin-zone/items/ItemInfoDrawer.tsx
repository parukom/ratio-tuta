                                ...
                                <div className="space-y-6 pb-16">
                                    ...
                                    <div>
                                        ...
                                        <div className="mt-4 flex items-start justify-between">
                                            <div>
                                                ...
                                                <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{item?.categoryName || 'No category'}</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <dl className="mt-2 divide-y divide-gray-200 border-t border-b border-gray-200 dark:divide-white/10 dark:border-white/10">
                                            ...
                                            <InfoRow label="Price" value={item ? fmtCurrency(item.price, currency) : '—'} />
                                            {typeof item?.pricePaid === 'number' && (
                                                <InfoRow label="Cost" value={fmtCurrency(item!.pricePaid!, currency)} />
                                            )}
                                            {typeof item?.pricePaid === 'number' && (
                                                <InfoRow label="Profit" value={fmtCurrency((item!.price - (item!.pricePaid || 0)), currency)} />
                                            )}
                                            <InfoRow label="Tax" value={item ? `${(item.taxRateBps / 100).toFixed(2)}%` : '—'} />
                                            <InfoRow label="Stock" value={String(item?.stockQuantity ?? 0)} />
                                            <InfoRow label="Unit" value={item?.unit || 'pcs'} />
                                            <InfoRow label="Measurement" value={item?.measurementType || 'PCS'} />
                                            ...
                                            <InfoRow label="Created" value={item ? new Date(item.createdAt).toLocaleString() : '—'} />
                                            <InfoRow label="Team" value={<code className="text-[11px]">{item?.teamId || '—'}</code>} />
                                            <InfoRow label="ID" value={<code className="text-[11px]">{item?.id || '—'}</code>} />
                                        </dl>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="mt-2">
                                            {item?.description ? (
                                                ...
                                                <p className="text-sm text-gray-500 italic dark:text-gray-400">No description.</p>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        {item?.tags && item.tags.length > 0 ? (
                                            ...
                                            <p className="mt-2 text-sm text-gray-500 italic dark:text-gray-400">No tags.</p>
                                        )}
                                    </div>
                                </div>
                            </div>
