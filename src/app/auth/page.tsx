    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        if (mode === "login") {
            try {
                ...
                const data = await res.json();
                if (!res.ok) {
                    // Identify unverified scenario if backend returns a recognizable code/message
                    const errMsg: string = data.error || "";
                    setMessage(errMsg || t("errors.login"));
                } else {
                    // Decide where to go next based on role and place memberships
                    const role = (data?.role as string) || "USER";
                    if (role === 'ADMIN') {
    ...
    return (
        <div className="relative flex min-h-full flex-col justify-center py-12 sm:px-6 lg:px-8">
            ...
            <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-[520px]">
                <div className="relative overflow-hidden rounded-2xl border bg-white/80 px-6 py-10 shadow-lg sm:px-12 dark:border-white/10 dark:bg-gray-900/60">
                    <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-transparent to-white/30 dark:to-black/20 mix-blend-overlay" />
                    {verifyMessage && showVerifyBanner && (
                        <div
                            className={`mb-6 rounded-md p-3 text-sm ${verifyStatus === 'success'
                                ? 'bg-green-50 text-green-800 dark:bg-green-900/20 dark:text-green-300'
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div>
                                ...
                                <div className="mt-2">
                                    <input
                                        ...
                                        value={name}
                                        onChange={(e) => setName(e.target.value)}
                                        className="block w-full rounded-xl bg-white/90 px-4 py-3 text-base text-gray-900 placeholder:text-gray-400 transition-shadow duration-150 outline-none ring-1 ring-transparent focus:ring-2 focus:ring-indigo-500 sm:text-sm/6 dark:bg-white/5 dark:text-white dark:placeholder:text-gray-500"
                                    />
                                </div>
                            </div>
                        )}
                        ...
                        <div>
                            <button
                                ...
                                disabled={submitting}
                                aria-busy={submitting}
                                className="flex w-full items-center justify-center gap-3 rounded-full bg-gradient-to-r from-indigo-600 to-indigo-500 px-6 py-3 text-sm/6 font-semibold text-white shadow-lg hover:opacity-95 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 disabled:opacity-60 disabled:cursor-not-allowed dark:from-indigo-500 dark:to-indigo-400"
                            >
                                {submitting && <Spinner size={18} className="text-white" />}
                                <span>{mode === "login" ? (submitting ? t("submit.loginProgress") : t("submit.login")) : (submitting ? t("submit.registerProgress") : t("submit.register"))}</span>
                            </button>
                                ...
                                <button
                                    ...
                                    onClick={async () => {
                                        ...
                                        try {
                                            ...
                                            const ok = res.ok;
                                            // Treat any 200 as success regardless of body to avoid enumeration
